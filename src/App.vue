...
window.addEventListener('load', () => {
	...
	if(path[2] === 'playtakviewer'){
		// plkaytakviewer
		console.log('playtak viewer')
	}else if(path[2] === 'ninjaviewer') {
		// ninjaviewer
		console.log('ninja viewer')
	}else if (path[2] === 'view' ) {
		// view
		...
		<q-header elevated class="bg-primary text-white">
			<q-toolbar>
				<q-toolbar-title>
					...
					Play Tak Game History Database
				</q-toolbar-title>
				<q-space />
				<q-btn stretch flat label="Play Tak" />
				<!-- TODO finish this -->
				<q-toggle v-model="lightMode" color="red" /><q-icon name="light_mode" size="large" />
			</q-toolbar>
		</q-header>

		<q-page-container class="container">
			<Search @search-event="searchGames" />
			<div class="row justify-center align-items-center q-mt-md">
				<q-spinner class="" color="primary" v-if="isLoading" size="5em"></q-spinner>
			</div>
			<TableComponent v-if="data && !isLoading" :data="data" />
		</q-page-container>
